...
const PlaceBidModal: React.FC<React.PropsWithChildren<PlaceBidModalProps>> = ({
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(bscTokens.doodz.address)
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient DOODZ balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% DOODZ or more.', { initialBidAmount }))
  ...
  return (
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      ...
      <InnerContent>
        ...
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% DOODZ or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
        ...
        <Text color="textSubtle" small mt="24px">
          {t('If your bid is unsuccessful, you’ll be able to reclaim your DOODZ after the auction.')}
        </Text>
      </InnerContent>
    </StyledModal>
  )
