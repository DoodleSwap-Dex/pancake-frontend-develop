...
const RoiCalculatorFooter: React.FC<React.PropsWithChildren<RoiCalculatorFooterProps>> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of DOODZ rewards each farm receives, as a proportion of the DOODZ produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 DOODZ per block, a 40x farm would receive 50 DOODZ per block.')}
      </Text>
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            {!Number.isFinite(apy) ? (
              <Text color="textSubtle" small>
                *{isFarm ? t('Base APR (DOODZ yield only)') : t('APR')}
              </Text>
            ) : (
              <Text color="textSubtle" small>
                {t('APY')}
