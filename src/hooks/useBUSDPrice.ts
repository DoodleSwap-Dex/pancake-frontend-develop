...
import { BUSD, DOODZ, USDC } from '@pancakeswap/tokens'
...
export const useCakeBusdPrice = ({ forceMainnet } = { forceMainnet: false }): Price<Currency, Currency> | undefined => {
  ...
  const doodz: Token = isTestnet ? DOODZ[ChainId.BSC_TESTNET] : DOODZ[ChainId.BSC]
  return usePriceByPairs(BUSD[doodz.chainId], doodz)
}
